<template>
    <AppLayout>
        <template #header>تعديل بيانات الموظف</template>
                ...
                <form @submit.prevent="submit" class="space-y-8">
                    ...
                    <div class="pt-6 border-t flex gap-4">
                        <button type="submit" :disabled="form.processing" class="px-8 py-3 rounded-xl font-bold text-white bg-black hover:bg-gray-800 shadow-md disabled:opacity-50">
                            {{ form.processing ? 'جاري التحديث...' : 'تحديث بيانات الموظف' }}
                        </button>
...
const form = useForm({
    ...
    transport_allowance: props.employee.transport_allowance,
    other_allowance: props.employee.other_allowance,
    
    overtime_calc_method: props.employee.overtime_calc_method,
    overtime_hourly_rate: props.employee.overtime_hourly_rate,
    overtime_multiplier: props.employee.overtime_multiplier,
    
    custom_grace_minutes: props.employee.custom_grace_minutes,
    
    bank_name: props.employee.bank_name || '',
    bank_account: props.employee.bank_account || '',
    bank_swift: props.employee.bank_swift || '',
