<template>
    <div class="print-wrapper" dir="rtl">
        <div class="no-print toolbar">
            ...
            <a :href="type==='agent'?'/agents':'/clients'" class="back-btn">← العودة</a>
        </div>
            ...
            <div class="overlay">
                <template v-if="pi === 0">
                    <div v-if="!isHidden('title')" class="field" :style="elPos('title')">
                        <span :style="elFont('title')">كشف حساب {{ type === 'agent' ? 'وكيل' : 'عميل' }}</span>
                    </div>
                    <div v-if="!isHidden('entity_name')" class="field" :style="elPos('entity_name')">
                        <span class="label">الاسم:</span><span class="value" :style="elFont('entity_name')">{{ entity.name }}</span>
                    </div>
                    <div v-if="!isHidden('entity_code')" class="field" :style="elPos('entity_code')">
                        <span class="label">الكود:</span><span class="value gold" :style="elFont('entity_code')">{{ entity.code }}</span>
                    </div>
                    <div v-if="!isHidden('period')" class="field" :style="elPos('period')">
                        <span class="label">الفترة:</span><span class="value" :style="elFont('period')">{{ filters.from }} → {{ filters.to }}</span>
                    </div>
                    <div v-if="!isHidden('currency')" class="field" :style="elPos('currency')">
                        <span class="label">العملة:</span><span class="value" :style="elFont('currency')">{{ type==='agent'?'SAR':'JOD' }}</span>
                    </div>
                ...
                <div v-if="pi > 0" class="page-num">
                    <span>كشف حساب: {{ entity.name }}</span>
                ...
                <div v-if="!isHidden('data_table')" :style="pi === 0 ? elPos('data_table') : contTablePos">
                    <table class="print-tbl" :style="{ fontSize: (el('data_table').fontSize||8)+'pt', width: el('data_table').w?el('data_table').w+'mm':'100%' }">
                        <thead><tr>
                            <th style="width:25px">#</th><th style="width:65px">التاريخ</th><th>الوصف</th><th style="width:50px">النوع</th><th style="width:65px">مدين</th><th style="width:65px">دائن</th><th style="width:70px">الرصيد</th>
                        </tr></thead>
                        <tbody>
                            <tr v-for="(e, i) in page.items" :key="e.id||i">
                ...
                <template v-if="page.isLast">
                    ...
                    <template v-for="(pos, id) in customFields" :key="id"><div v-if="!pos.hidden" class="field" :style="elPos(id)"><span :style="elFont(id)" style="white-space:pre-wrap">{{ replaceVars(pos.text||'') }}</span></div></template>
                </template>
            </div>
        </div>
    </div>
...
const defaults = {
    title:{x:10,y:15,fontSize:14}, entity_name:{x:10,y:28,fontSize:12}, entity_code:{x:120,y:28,fontSize:10},
    period:{x:200,y:28,fontSize:10}, currency:{x:200,y:15,fontSize:10}, summary_box:{x:10,y:40,fontSize:9,w:277},
...
const sigPos = (page, pi) => {
    ...
    const y = baseY + 7 + (page.items.length * 5.5) + 10;
...
const replaceVars = (text) => {
    ...
    const map = { '{{اسم_العميل}}':e?.name||'', '{{كود_العميل}}':e?.code||'', '{{هاتف_العميل}}':e?.phone||'', '{{الفترة}}':`${props.filters?.from} → ${props.filters?.to}` };
...
.summary-row{display:flex;gap:8px}.sum-card{flex:1;background:#f9f9f9;padding:6px 10px;border-radius:6px;text-align:center;border:1px solid #eee}
.sum-card.debit{background:#fef2f2;border-color:#fecaca}.sum-card.credit{background:#f0fdf4;border-color:#bbf7d0}.sum-card.closing{background:#fffbeb;border-color:#fde68a}
.sum-label{font-size:7pt;color:#999;margin:0 0 2px}.sum-val{margin:0;font-weight:700;font-size:9pt}
...
.print-tbl td{padding:3px 6px;border-bottom:.5px solid #e8e4de;font-size:7.5pt}.print-tbl .center{text-align:center}.print-tbl .right{text-align:left;direction:ltr}
